...
void	test_intern(std::string const & form_name, std::string const & target) {
	std::cout << "\n##### Testing Intern with " <<
		form_name << " form on " << target << "\n";
	Intern	someRandomIntern;
	Bureaucrat	bob("Bob", 1);
	Form*	form_ptr;

	form_ptr = someRandomIntern.makeForm(form_name, target);

	if (form_ptr) {
		std::cout << bob;
		bob.signForm(*form_ptr);
		std::cout << *form_ptr;
		bob.executeForm(*form_ptr);
	}
...
int	main() {
	...
	test_officeblock_no_param("shrubbery creation", "Hello");
	test_officeblock_no_param("robotomy request", "Hello");
